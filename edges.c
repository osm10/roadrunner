...
int vc_gray_edge_prewitt(IVC *src, IVC *dst, int threshold) {
  ...
  for (int x = 1; x < src->width - 1; x++) {
    for (int y = 1; y < src->height - 1; y++) {
      ...
      my = ((datasrc[posF] + datasrc[posG] + datasrc[posH]) -
            (datasrc[posA] + datasrc[posB] + datasrc[posC])) /
           3;
      mx = ((datasrc[posC] + datasrc[posE] + datasrc[posH]) -
            (datasrc[posA] + datasrc[posD] + datasrc[posF])) /
           3;
