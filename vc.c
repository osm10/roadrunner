...
IVC *vc_rgb_new(int width, int height) {
  return vc_image_new(width, height, 3, 255);
}

IVC *vc_grayscale_new(int width, int height) {
  return vc_image_new(width, height, 1, 255);
}

IVC *vc_binary_new(int width, int height) {
  return vc_image_new(width, height, 1, 1);
}
...
int vc_write_image(char *filename, IVC *image) {
  ...
  if ((file = fopen(filename, "wb")) != NULL) {
    if (image->levels == 1) // se a imagem for binária
    {
      sizeofbinarydata =
      ...
      // grava o header de uma imagem binária
      fprintf(file, "%s %d %d\n", "P4", image->width, image->height);

      // converte os pixeis para bits
      totalbytes =
      ...
      free(tmp);
    } else // se a imagem from em escala de cinzentos ou a cores
    {
      // grava o header de uma imagem não binária
...
int vc_write_image_info(char *filename, IVC *image) {
  ...
  return vc_write_image(filename, image);
}

int vc_is_binary(IVC *image) {
  return ((image->levels == 1) && (image->channels == 1));
}

int vc_is_grayscale(IVC *image) {
  return ((image->levels == 255) && (image->channels == 1));
}

int vc_is_rgb(IVC *image) {
  return ((image->levels == 255) && (image->channels == 3));
}
